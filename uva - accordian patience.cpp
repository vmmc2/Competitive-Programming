...
int main(){
    ...
    while(getline(cin, aux)){
            if(aux == "#") break; //fim dos casos de teste
                ...
                int can = 0;//suponho que não posso mais fazer jogadas;
                while(can == 0){
                    can = 0;
                    for(int i = 0; i < (int)pilhas.size(); i++){
                        if(i <= 2) continue;
                        else{
                            if(check(pilhas[i - 3].top(), pilhas[i].top())){
                                ...
                                continue;
                            }else if(check(pilhas[i - 1].top(), pilhas[i].top())){
                                string m1 = pilhas[i].top();
                                pilhas[i - 1].push(m1);
                                can = 1;
                                if(pilhas[i].empty()){
